...
// import sendMessage, { setCallback } from './utils/sendMessage';
    ...
    return new Observable((resolve) => {
        new ModelLoader().load(
            ...
            (geometry) => {
                ...
                resolve.next({ type: 'LOAD_MODEL_POSITIONS', positions, originalPosition });
                ...
                resolve.next({ type: 'LOAD_MODEL_CONVEX', positions: convexPositions });
                resolve.complete();
            },
            (progress) => {
                resolve.next({ type: 'LOAD_MODEL_PROGRESS', progress });
            },
            (err) => {
                resolve.next({ type: 'LOAD_MODEL_FAILED', err });
                resolve.error();
            }
        );
    });
