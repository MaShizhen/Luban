...
class SVGCanvas extends PureComponent {
    static propTypes = {
        ...
        onDrawTransformStart: PropTypes.func.isRequired,
    ...
    setupMouseEvents() {
        ...
        this.svgContainer.addEventListener('mouseleave', (event) => {
            ...
            this.calculateSelectedModel(event, 'mouseleave');
        }, false);
        window.addEventListener('resize', this.onResize, false);
    ...
    calculateSelectedModel = (event, mouseleave) => {
    ...
    onMouseUp = (event) => {
        if (this.mode === 'select' && event.which === 1 && !this.editingElem) {
            this.calculateSelectedModel(event);
        }
        const draw = this.currentDrawing;
        if (!draw.started) {
            return;
    ...
    onDblClick = (evt) => {
        ...
        if (this.props.editable && tagName === 'text' && this.mode !== 'textedit') {
            ...
            this.setMode('select');
            this.svgContentGroup.drawGroup.startDraw(this.mode, mouseTarget, svgModel.transformation);
            this.props.onDrawTransformStart(mouseTarget);
        }
    };
    ...
    startDraw = () => {
        if (this.editingElem) {
            ...
            this.setMode('draw');
            this.svgContentGroup.drawGroup.startDraw(this.mode);
        }
    };
        ...
        if (!exitCompletely && elem && mode === 'draw') {
            ...
            const loop = setInterval(() => {
                ...
                if (svgModel) {
                    ...
                    this.props.onDrawTransformStart(elem);
                }
            }, 100);
        }
    };
